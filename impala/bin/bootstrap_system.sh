...
if [[ -f /etc/redhat-release ]]; then
  ...
  then
    ...
    then
      ...
      echo "This script only supports 16.10 or 18.04 of Ubuntu" >&2
      exit 1
    fi
  else
...
# Helper function to execute following command only on Ubuntu 16.10
function ubuntu16 {
...
if [[ $UBUNTU = true && ( $DISTRIB_RELEASE = 16.10 || $DISTRIB_RELEASE = 18.04 ) ]]
then
